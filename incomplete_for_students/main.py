...
class SimpleRobotControl:
    ...
    def play(self):
        ...
        while True:
            ...
            if self.is_artist:
                ...
                self.screen.blit(
                    self.small_font.render(
                        "X speed: {0:.3f} rad/s".format(self.m.x_goal),
                        1,
                        (0, 150, 0),
                    ),
                    [20, 240],
                ...
                self.screen.blit(
                    self.small_font.render(
                        "Y speed: {0:.3f} rad/s".format(self.m.y_goal),
                        1,
                        (0, 150, 0),
                    ),
                    [20, 270],
    ...
    def asserv(self, m=None):
        ...
        #origin relative to the robot
        x = m.x_goal - m.x
        y = m.y_goal - m.y

        angle_goal = 2*atan(y/(x + sqrt(x*x + y*y)))
        print(f"angle goal:{angle_goal}")
        # TODO
        rotation_speed = self.m.dk()
        local_speed = distance
        if rotation_speed != 0:

            local_turn = self.angle_diff(angle_goal, m.theta)/local_speed
    ...
    def angle_diff(self, a, b):
        ...
        """
        # TODO
        d = atan2(math.sin(a-b), math.cos(a-b))
        return d
